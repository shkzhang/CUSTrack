...
from core.models.custrack.utils import combine_tokens, recover_tokens
...
class BaseBackbone(nn.Module):
    ...
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        patch_pos_embed = patch_pos_embed.transpose(1, 2).contiguous()
        ...
        search_patch_pos_embed = nn.functional.interpolate(patch_pos_embed, size=(new_P_H, new_P_W), mode='bicubic',
                                                           align_corners=False)
        search_patch_pos_embed = search_patch_pos_embed.flatten(2).transpose(1, 2).contiguous()
        ...
        template_patch_pos_embed = nn.functional.interpolate(patch_pos_embed, size=(new_P_H, new_P_W), mode='bicubic',
                                                             align_corners=False)
        template_patch_pos_embed = template_patch_pos_embed.flatten(2).transpose(1, 2).contiguous()

        self.pos_embed_z = nn.Parameter(template_patch_pos_embed)
        self.pos_embed_x = nn.Parameter(search_patch_pos_embed)
        self.constant_input_z = nn.Parameter(template_patch_pos_embed)
        self.constant_input_x = nn.Parameter(search_patch_pos_embed)
