...
from core.models.layers.frozen_bn import FrozenBatchNorm2d
...
class Corner_Predictor(nn.Module):
    ...
    def get_score_map(self, x):
        ...
        # size branch
        # x_offset1 = self.conv1_size(x)
        # x_offset2 = self.conv2_size(x_offset1)
        # x_offset3 = self.conv3_size(x_offset2)
        # x_offset4 = self.conv4_size(x_offset3)
        # offset = self.conv5_size(x_offset4)
        return score_map_tl, score_map_br

    def cal_bbox(self, score_tl,  score_br):
        prob_vec_tl = score_tl.view((-1, self.feat_sz * self.feat_sz))  # (batch, feat_sz * feat_sz)
        prob_vec_br = score_br.view((-1, self.feat_sz * self.feat_sz))  # (batch, feat_sz * feat_sz)

        coorx_tl, coory_tl, prob_vec_tl = self.soft_argmax(prob_vec_tl, return_dist=True)
        coorx_br, coory_br, prob_vec_br = self.soft_argmax(prob_vec_br, return_dist=True)
        return torch.stack((coorx_tl, coory_tl, coorx_br, coory_br), dim=1) / self.img_sz
    ...
    def soft_argmax(self, score_map, return_dist=False, softmax=True):
        ...
        if return_dist:
            if softmax:
                ...
                return exp_x, exp_y, _sigmoid(score_map)
        else:
            return exp_x, exp_y
...
class CenterPredictor(nn.Module, ):
    ...
    def forward(self, x, gt_score_map=None):
        """ Forward pass with input x. """
        score_map_ctr, size_map, offset_map,_score_map_ctr = self.get_score_map(x)
        ...
        return score_map_ctr, bbox, size_map, offset_map,_score_map_ctr
    ...
    def cal_bbox(self, score_map_ctr, size_map, offset_map, return_score=False):
        ...
        flatten_size_map = size_map.flatten(2)
        size = torch.gather(flatten_size_map,dim=2,index=idx)
        size = torch.squeeze(size,-1)
    ...
    def get_score_map(self, x):
        ...
        return _sigmoid(score_map_ctr), _sigmoid(score_map_size), score_map_offset,score_map_ctr
...
class CenterPointPredictor(nn.Module, ):
    ...
    def forward(self, x, gt_score_map=None):
        """ Forward pass with input x. """
        score_map_ctr,effect_map = self.get_score_map(x)
        return score_map_ctr,effect_map
    ...
    def get_score_map(self, x):
        ...
        x_ctr4 = self.conv4_ctr(x_ctr3)
        score_map_ctr = self.conv5_ctr(x_ctr4)
        return _sigmoid(score_map_ctr),score_map_ctr
...
def build_box_head(cfg, hidden_dim,point_only=False):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        feat_sz = int(cfg.DATA.SEARCH.SIZE / stride)
        if point_only:
            center_head = CenterPointPredictor(inplanes=in_channel, channel=out_channel,
                                          feat_sz=feat_sz, stride=stride)
        else:
