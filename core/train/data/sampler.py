...
from core.utils import TensorDict
import numpy as np
...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, seq_enhance_ratio=0, negative_ratio=0):
        """
        args:
        ...
        self.seq_enhance_ratio = seq_enhance_ratio
        self.negative_ratio = negative_ratio
        self.sample_num = 0
        assert self.max_gap >= self.num_search_frames, "Sample gap must be greater than or equal to number of search frames"
    def __len__(self):
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None,
        ...
        if len(valid_ids)>num_ids:
            return np.random.choice(valid_ids, num_ids,replace=False).tolist()
        else:
            return np.random.choice(valid_ids, num_ids).tolist()
        ...
        while not valid:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    while search_frame_ids is None:
                        base_frame_id = self._sample_visible_ids(visible, num_ids=1,
                                                                 ...
                                                                 max_id=len(visible) - num_search_frames)
                        ...
                        if random.random() < self.seq_enhance_ratio:
                            search_frame_ids.sort(reverse=True)
                        gap_increase += 5
            ...
            try:
                ...
                data = TensorDict({'template_images': template_frames,
                                   ...
                                   "template_frame_ids": torch.Tensor(template_frame_ids),
                                   "search_frame_ids":torch.Tensor(search_frame_ids),
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset,skip_id=None):
        ...
        while not enough_visible_frames:
            ...
            if skip_id is not None:
                choices = [i for i in range(dataset.get_num_sequences()) if i != skip_id]
                weights = [w for i,w in enumerate(dataset.get_sample_weights()) if i != skip_id]
                seq_id = random.choices(choices,k=1,weights=weights)[0]
            else:
                choices = [i for i in range(dataset.get_num_sequences())]
                seq_id = random.choices(choices,k=1,weights=dataset.get_sample_weights())[0]
