...
def create_default_local_file_ITP_train(workspace_dir, data_dir, save_dir):
    path = os.path.join(os.path.dirname(__file__), 'local.py')
    ...
    default_settings = OrderedDict({
        ...
        'clust_dir':os.path.join(data_dir, 'clust'),
        'ndth_dir':os.path.join(data_dir, 'US-Liver-Track-Dataset'),
        'data_dir': data_dir,
        'save_dir': save_dir,
...
def env_settings():
    env_module_name = 'core.train.admin.local'
    try:
