...
from core.train.data.image_loader import jpeg4py_loader_w_failsafe
...
class BaseVideoDataset(torch.utils.data.Dataset):
    ...
    def add_weight_sample(self):
        self.weights_sample = []
        for i in range(len(self.sequence_list)):
            info = self.get_sequence_info(i)
            valid = info['valid']
            num = len(valid)
            self.weights_sample.append(num)
        sample_sum = np.sum(self.weights_sample)
        self.weights_sample = [i / sample_sum for i in self.weights_sample]

    def get_sample_weights(self):
        return self.weights_sample if len(self.weights_sample)!=0 else None
