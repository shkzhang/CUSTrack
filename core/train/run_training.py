...
def run_training(script_name, config_name, cudnn_benchmark=True, local_rank=-1,base_seed=None,
    ...
    args:
        script_name: Name of emperiment in the "configs/" folder.
        config_name: Name of the yaml file in the "configs/<script_name>".
    ...
    print('Run script: {}.py with config: {}.yaml'.format(script_name, config_name))
    ...
    settings.save_dir = settings.env.save_dir
    settings.use_lmdb = use_lmdb
    prj_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
    settings.cfg_file = os.path.join(prj_dir, 'configs/%s/%s.yaml' % (script_name, config_name))
    ...
    if distill:
        ...
        settings.cfg_file_teacher = os.path.join(prj_dir, 'configs/%s/%s.yaml' % (script_teacher, config_teacher))
        expr_module = importlib.import_module('core.train.train_script_distill')
    else:
        expr_module = importlib.import_module('core.train.train_script')
...
def main():
    ...
    parser.add_argument('--script', default='custrack', type=str, help='Name of the train script.')
    ...
    parser.add_argument('--use_wandb', default=0, type=int, choices=[0, 1])  # whether to use wandb
