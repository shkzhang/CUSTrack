...
from core.train.admin import multigpu
...
class BaseTrainer:
    ...
    def update_settings(self, settings=None):
        ...
        if self.settings.env.workspace_dir is not None:
            ...
            logger.info("checkpoints will be saved to %s" % self._checkpoint_dir)

            if self.settings.local_rank in [-1, 0]:
                if not os.path.exists(self._checkpoint_dir):
                    logger.info("Training with multiple GPUs. checkpoints directory doesn't exist. "
        ...
        for i in range(num_tries):
            ...
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if epoch > (max_epochs - 1) or save_every_epoch or epoch % self.settings.checkpoint_epoch_interval == 0  or epoch > (max_epochs - 5):
                        if self._checkpoint_dir:
                            if self.settings.local_rank in [-1, 0]:
                                self.save_checkpoint()
            except:
                logger.error('Training crashed at epoch {}'.format(epoch))
                if fail_safe:
                    self.epoch -= 1
                    load_latest = True
                    logger.error('Traceback for the error!')
                    logger.error(traceback.format_exc())
                    logger.error('Restarting training from last epoch ...')
                else:
                    raise
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        if checkpoint is None:
            ...
            if checkpoint_list:
                ...
                logger.info('No matching checkpoint file found')
                return
