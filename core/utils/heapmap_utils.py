...
def generate_heatmap(bboxes, patch_size=320, stride=16):
    ...
    Args:
        bboxes (torch.Tensor): shape of [bs, 4]
    ...
    return gt_scoremap.to(bbox.device)
def generate_corner_heatmap(bboxes, patch_size=320, stride=16):
    """
    Generate ground truth heatmap same as CenterNet
    Args:
        bboxes (torch.Tensor): shape of [bs, 4]
    ...
    heatmap_size = patch_size // stride
    bs = bboxes.shape[0]
    gt_scoremap_tl = torch.zeros(bs, heatmap_size, heatmap_size)
    gt_scoremap_br = torch.zeros(bs, heatmap_size, heatmap_size)

    classes = torch.arange(bs).to(torch.long)
    bbox = bboxes * heatmap_size
    wh = bbox[:, 2:]
    centers_tl = (bbox[:, :2]).round()
    centers_br = (bbox[:, 2:]).round()

    CenterNetHeatMap.generate_score_map(gt_scoremap_tl, classes, wh, centers_tl, 0.7)
    CenterNetHeatMap.generate_score_map(gt_scoremap_br, classes, wh, centers_br, 0.7)

    return gt_scoremap_tl.to(bbox.device),gt_scoremap_br.to(bbox.device)
