...
def calc_err_center(pred_bb, anno_bb, normalized=False):
    ...
    return err_center
def calc_err_center_xy(pred_bb, anno_bb, normalized=False):
    pred_center = pred_bb[:, :2] + 0.5 * (pred_bb[:, 2:] - 1.0)
    anno_center = anno_bb[:, :2] + 0.5 * (anno_bb[:, 2:] - 1.0)
    ...
    err_center_xy = pred_center - anno_center
    return err_center_xy
...
def calc_seq_err_robust(pred_bb, anno_bb, dataset, target_visible=None):
    ...
    err_center_xy=calc_err_center_xy(pred_bb,anno_bb)
    ...
    return err_overlap, err_center, err_center_normalized,err_center_xy, valid


def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
                    exclude_invalid_frames=False,return_frame_results=False):
    ...
    ave_success_rate_plot_center_norm = torch.zeros((len(dataset), len(trackers), threshold_set_center.numel()),
                                                    dtype=torch.float32)
    ave_center_error = torch.zeros((len(dataset), len(trackers), 1), dtype=torch.float32)
    ave_iou = torch.zeros((len(dataset), len(trackers), 1), dtype=torch.float32)
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            invalid_rows = np.where((anno_bb == -1).all(axis=1))[0]


            assert anno_bb.shape == pred_bb.shape,"The predicted result is inconsistent with the ground truth!"
            anno_bb = np.delete(anno_bb, invalid_rows, axis=0)
            pred_bb = np.delete(pred_bb, invalid_rows, axis=0)
            if trk_id==0:
                seq_ground_truth.append(anno_bb)
            # Calculate measures
            err_overlap, err_center, err_center_normalized,err_center_xy, valid_frame = calc_seq_err_robust(
            ...
            if return_frame_results:
                seq_frames_dist.append(pred_bb)
            ...
            ave_center_error[seq_id,trk_id,:] = err_center.mean(0)
            ave_iou[seq_id,trk_id,:] = err_overlap.mean(0)
        if return_frame_results:
            seq_frames_dist = torch.stack(seq_frames_dist)
            frames_dist.append(seq_frames_dist)
    ...
    eval_data = {'sequences': seq_names, 'trackers': tracker_names,
                 ...
                 'ave_center_error':ave_center_error.tolist(),
                 'ave_iou':ave_iou.tolist(),
                 'frames_dist':frames_dist,
                 'ground_truth_dist':seq_ground_truth
                 }
